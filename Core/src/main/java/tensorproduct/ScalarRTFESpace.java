	...
	TPFace,TPEdge,RTComponentFunction,Double, CoordinateVector, Matrix>, Assembleable
{
	...
	TreeMultimap<TPCell,RTComponentFunction> supportOnCell;
	TreeMultimap<TPFace,RTComponentFunction> supportOnFace;
	Set<RTComponentFunction> shapeFunctions;
	...
	public void assembleFunctions(int polynomialDegree)
	{
		...
		for(TPCell cell: cells)
			for(int i = 0; i < Math.pow(polynomialDegree+1, dimension-1)*(polynomialDegree+2); i++)
			{
	...
	@Override
	public Map<Integer, RTComponentFunction> getShapeFunctions()
	{
		Map<Integer, RTComponentFunction> functionNumbers = new TreeMap<>();
		for(RTComponentFunction shapeFunction:shapeFunctions)
	...
	@Override
	public Set<RTComponentFunction> getShapeFunctionsWithSupportOnCell(TPCell cell)
	{
		return supportOnCell.get(cell);
	}
	
	@Override
	public Set<RTComponentFunction> getShapeFunctionsWithSupportOnFace(TPFace face)
	{
