...
public class TPFaceIntegral<ST extends ScalarShapeFunction<TPCell, TPFace>>
	extends FaceIntegral<TPFace, ST>
{
	...
	public TPFaceIntegral(final double weight, final String name, final QuadratureRule1D quadratureRule1D)
	{
		this(ScalarFunction.constantFunction(weight), name, quadratureRule1D);
	}
	
	public TPFaceIntegral(final double weight, final String name)
	{
		this(ScalarFunction.constantFunction(weight), name);
	}
	
	public TPFaceIntegral(final Function<?, ?, ?> weight, final String name)
	{
		this(weight, name, QuadratureRule1D.Gauss5);
	}
	
	public TPFaceIntegral(final Function<?, ?, ?> weight,
	                      final String name,
	...
	public TPFaceIntegral(final String name)
	{
		this(name, QuadratureRule1D.Gauss5);
	}
	
	public TPFaceIntegral(final String name, final QuadratureRule1D quadratureRule1D)
	{
		...
		return TPCellIntegral.integrateNonTensorProduct(x ->
		                                                {
			                                                ...
			                                                return eval.applyAsDouble(CoordinateVector.fromValues(
				                                                point));
		                                                }, cells, quadratureRule);
	}
	
	@Override
	public double evaluateFaceIntegral(final TPFace face, final ST shapeFunction1,
	                                   final ST shapeFunction2)
	{
		if (name.equals(VALUE_JUMP_VALUE_JUMP))
		{
			return integrateNonTensorProduct(x -> shapeFunction1.jumpInValue(face,
			                                                                 x) * shapeFunction2.jumpInValue(
				                                 face,
		...
		{
			return integrateNonTensorProduct(x -> shapeFunction1.normalAverageInDerivative(face,
			                                                                               x) * shapeFunction2.jumpInValue(
				                                 face,
				                                 x) * (Double) weight.value(x),
			                                 face,
		...
		{
			return integrateNonTensorProduct(x -> shapeFunction1.jumpInValue(face,
			                                                                 x) * shapeFunction2.normalAverageInDerivative(
				                                 face,
				                                 x) * (Double) weight.value(x),
			                                 face,
			                                 quadratureRule1D);
		...
		if (name.equals(GRAD_VALUE_NORMAL))
		{
			return integrateNonTensorProduct(x -> shapeFunction1.gradient(x)
			                                                    .inner(face.getNormal()
			                                                               .value(x)) * shapeFunction2.value(
				                                 x) * (Double) weight.value(x),
			                                 face,
			                                 quadratureRule1D);
		}
		...
		if (name.equals(VALUE_VALUE))
		{
			return integrateNonTensorProduct(x ->
				                                 shapeFunction1.value(x) * shapeFunction2.value(x) * (Double) weight.value(
					                                 x),
			                                 face,
			                                 quadratureRule1D);
		}
		if (name.equals(BOUNDARY_VALUE))
		{
			...
			{
				return integrateNonTensorProduct(x ->
					                                 shapeFunction1.jumpInValue(face,
					                                                            x) * shapeFunction2.jumpInValue(
						                                 face,
						                                 x) * (Double) weight.value(x),
				                                 face,
