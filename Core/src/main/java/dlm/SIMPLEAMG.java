	...
	implements Smoother
{
	...
	public SIMPLEAMG(final int runs, final double omega, final int vSize,
	                     ...
	                     final boolean verbose, final String prefix)
	{
		...
		if (Ainv == null)
		{
			...
			A = blocks[0];
			C = blocks[3];
			final SparseMatrix Apadded = new SparseMatrix(tot_size, tot_size);
			Apadded.addSmallMatrixInPlaceAt(SparseMatrix.identity(tot_size - vel_size), vel_size, vel_size);
			Apadded.addSmallMatrixInPlaceAt(A, 0, 0);
			final VectorMultiplyable amgA = mg.AMGFromMatrix(Apadded,
			...
			Ainv = new VectorMultiplyable()
			{
				...
				public Vector mvMul(final Vector vector)
				{
					...
					v.addSmallVectorAt(vector, 0);
					final Vector ainvV = amgA.mvMul(v);
					final Vector redV = ainvV.slice(0, vel_size);
					System.out.println(A.mvMul(redV)
					                    .sub(vector)
					                    .euclidianNorm());
					return redV;
				}
