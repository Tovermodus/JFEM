	...
	implements MutableMatrix, DirectlySolvable
{
	...
	public UpperTriangularMatrix(final int rows, final int cols)
	{
		if (rows != cols)
	...
	public UpperTriangularMatrix(final Matrix matrix)
	{
	...
	public UpperTriangularMatrix add(final UpperTriangularMatrix other)
	{
		return new UpperTriangularMatrix(underlying.add(other.underlying));
	}
	...
	@Override
	public UpperTriangularMatrix mul(final double scalar)
	{
		return new UpperTriangularMatrix(underlying.mul(scalar));
	}
	...
	{
		return new LowerTriangularMatrix(underlying.transpose());
	}
	...
	{
		if (!(other instanceof UpperTriangularMatrix))
			throw new IllegalArgumentException("other needs to be lower triangular");
		underlying.addInPlace(((UpperTriangularMatrix) other).underlying);
	}
