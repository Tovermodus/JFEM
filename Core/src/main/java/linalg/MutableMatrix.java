...
public interface MutableMatrix extends Matrix, MutableTensor
{
	...
	default void addSmallMatrixAt(Matrix small, int...coordinates)
	{
	...
	default void addColumn(Vector vector, int column)
	{
		addColumn(vector, column, 0);
	}
	default void addColumn(Vector vector, int column, int start)
	{
		if (PerformanceArguments.getInstance().executeChecks)
		{
		...
		for(IntCoordinates c:vector.getShape().range())
		{
			add(vector.at(c), c.get(0) + start, column);
		}
		
	}
	default void addRow(Vector vector, int row)
	{
		addRow(vector, row, 0);
	}
	default void addRow(Vector vector, int row, int start)
	{
		if (PerformanceArguments.getInstance().executeChecks)
		{
