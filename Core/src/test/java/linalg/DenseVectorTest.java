...
public class DenseVectorTest
{
	...
	public void testGetters() {
		DenseVector vector = createVector();
		DenseVector vector2 = createVector();
	...
	{
		DenseVector xv = DenseVector.vectorFromValues(2,3,2);
		DenseVector yv = DenseVector.vectorFromValues(9,8,9);
		DenseMatrix comp = DenseMatrix.squareMatrixFromValues(2*9,3*9,2*9,
	...
	{
		DenseVector vector = createVector();
		double val = vector.inner(vector.mul(0.0002));
		double valcomp = IntStream.range(0,n).mapToDouble((i)->i*i*0.0002).sum();
		assertTrue(Math.abs(val- valcomp) < 1e-15*n*n);
	}
	@Test
	public void testMatrixVectorProduct()
	{
		DenseVector v = createVector();
		SparseMatrix s = SparseMatrix.identity(n);
