...
public class StokesMGTransposed2
{
	public static void main(final String[] args)
	{
			                                 ...
			                                 1)
		{
			...
			public List<Smoother> createSmoothers()
			{
				...
				{
					ret.add(new RichardsonSmoother(0.005,
					                               8,
					                               systems.get(i)
					                                      .transpose()));//, d
		...
		PlotWindow.addPlot(new ScalarPlot2D(LaplaceReferenceSolution.scalarReferenceSolution(),
		                                    mg.spaces.get(refinements)
		                                             .generatePlotPoints(30), 30, "reference"));
		
		final GMRES2 gm = new GMRES2(1e-8, true);
		final GMRES2 gm2 = new GMRES2(1e-8, false);
		gm.verbose = false;
		gm2.verbose = false;
		final Vector solution;// = gm.solve(mg.finest_system, mg.finest_rhs);
		
		solution = gm.solve(mg.finest_system, mg,
		                    mg.finest_rhs);
